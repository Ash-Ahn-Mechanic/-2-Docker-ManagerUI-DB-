...
from manager_ui.backend.db.models import ErrorLog, CommandLog
...
def get_recommendations(limit: int = Query(30, le=100), db: Session = Depends(get_db)):
    ...
    for err in errors:
        ...
        action_name = None
        if err.command_id:
            doc = mongo.get_command(err.command_id)
            if doc:
                actions = doc.get('action_params', [])
                hit = next(
                    (a for a in actions if a.get('status') in ('executing', 'failed')),
                    actions[-1] if actions else None,
                )
                action_name = hit.get('action') if hit else None
